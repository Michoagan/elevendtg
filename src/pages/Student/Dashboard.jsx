                                        ...
                                        filteredEpreuves.map(epreuve => (
                                            <div key={epreuve.id} className="card hover-shadow" style={{ display: 'flex', flexDirection: 'column', padding: '1.5rem' }}>
                                                ...
                                                <div className="flex gap-2 mt-auto pt-4 border-t" style={{ borderColor: 'hsl(var(--text-dark)/0.1)' }}>
                                                    <a href={`${import.meta.env.VITE_API_BASE_URL}/storage/${epreuve.file_path}`} target="_blank" rel="noopener noreferrer" className="flex-1 btn btn-secondary text-center" style={{ padding: '0.5rem', fontSize: '0.9rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '5px' }}>
                                                        <Eye size={16} /> Voir
                                                    </a>
                                                    <a href={`${import.meta.env.VITE_API_BASE_URL}/storage/${epreuve.file_path}`} download className="flex-1 btn btn-outline text-center" style={{ padding: '0.5rem', fontSize: '0.9rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '5px' }}>
